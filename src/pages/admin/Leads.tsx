...
import { MessageSquare, Phone, Mail, Eye, Edit2, Trash2 } from "lucide-react";
...
interface Enquiry {
  ...
  phone: string;
  message: string;
  propertyId?: { _id: string; title: string; slug: string };
...
const Leads = () => {
  ...
  const handleStatusChange = async (enquiryId: string, newStatus: "new" | "reviewed" | "closed") => {
    try {
  ...
  const handleDeleteEnquiry = async (enquiryId: string) => {
    ...
    try {
      await api.deleteEnquiry(enquiryId);
  ...
  const filteredEnquiries = enquiries.filter(enquiry => {
    const matchStatus = filterStatus === "all" || enquiry.status === filterStatus;
    const matchSearch = enquiry.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                        ...
                        <TableRow key={enquiry._id}>
                          ...
                          <TableCell className="text-sm">
                            <div className="flex flex-col gap-1">
                              ...
                              {enquiry.email && (
                                <a href={`mailto:${enquiry.email}`} className="text-blue-600 hover:underline text-xs">
                                  {enquiry.email}
                                </a>
                              )}
                            </div>
                          </TableCell>
                          <TableCell className="text-sm max-w-xs truncate">{enquiry.message}</TableCell>
                          <TableCell className="text-sm">
                            {enquiry.propertyId ? enquiry.propertyId.title : "General"}
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell>
                            <div className="flex gap-2">
                              <Dialog open={openDetail && selectedEnquiry?._id === enquiry._id} onOpenChange={setOpenDetail}>
                                    ...
                                    <div className="space-y-4">
                                      ...
                                      <div className="grid grid-cols-2 gap-4">
                                        <div>
                                          ...
                                          <a href={`https://wa.me/91${selectedEnquiry.phone.slice(-10)}`} target="_blank" rel="noopener noreferrer">
                                      ...
                                      <div>
                                        ...
                                        <p className="text-sm bg-muted p-2 rounded">{selectedEnquiry.message}</p>
                                      </div>
                                      {selectedEnquiry.propertyId && (
                                        <div>
                                          <Label className="text-muted-foreground">Related Property</Label>
                                          <p className="text-sm font-medium">{selectedEnquiry.propertyId.title}</p>
                                        </div>
                                      )}
                                      <div>
                                        <Label className="text-muted-foreground">Date</Label>
                                        <p className="text-sm">{new Date(selectedEnquiry.createdAt).toLocaleString()}</p>
                                      </div>
